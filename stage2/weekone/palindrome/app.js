...
function firstAlgorithm(userInput) {
  userInput = str.toLowerCase().replace(re, "");
  let palCheck = userInput.split("").reverse().join("");
  if (palCheck == userInput) return true;
  else return false;
}

function secondAlgorithm(userInput) {
  let palCheck = user;
}
